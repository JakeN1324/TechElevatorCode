    ...
    public class AuthenticatedApiService
    {
        ...
        protected void CheckForError(IRestResponse response)
        {
            ...
            else if (!response.IsSuccessful)
            {
                ...
                {
                    message = $"An authorization error occurred. Status code {(int)response.StatusCode} {response.StatusDescription}";
                    throw new HttpRequestException(message, response.ErrorException);
                }
                else if (response.StatusCode == System.Net.HttpStatusCode.Forbidden)
                {
                    message = $"User does not have permission. Status code {(int)response.StatusCode} {response.StatusDescription}";
                    throw new HttpRequestException(message, response.ErrorException);
                }
                else
                {
